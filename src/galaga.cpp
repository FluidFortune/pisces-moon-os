...
// Non-blocking explosions
#define MAX_EXPLOSIONS 6
struct Explosion {
    int16_t x, y;
    int8_t  frame;     // 0 = not active, 1-5 = animation frames
};
static Explosion explosions[MAX_EXPLOSIONS];

// Trigger a non-blocking explosion (replaces blocking drawExplosion)
static void triggerExplosion(int x, int y) {
...
static void updateExplosions() {
    static const uint16_t cols[] = {0xF800, 0xFD20, 0xFFE0, 0xFFFF, 0xC638};
    for (int i = 0; i < MAX_EXPLOSIONS; i++) {
        if (explosions[i].frame == 0) continue;
        int x = explosions[i].x;
        int y = explosions[i].y;
        int f = explosions[i].frame;
        // Erase previous radius
        if (f > 1) {
            gfx->drawCircle(x, y, (f-1) * 2, COL_BG);
        }
        if (f <= 5) {
            gfx->drawCircle(x, y, f * 2, cols[f-1]);
            explosions[i].frame++;
        } else {
            // Final cleanup
            gfx->fillRect(x - 12, y - 12, 24, 24, COL_BG);
            // Restore stars in patch
            for (int s = 0; s < NUM_STARS; s++) {
                if (stars[s].x >= x-12 && stars[s].x <= x+12 &&
                    stars[s].y >= y-12 && stars[s].y <= y+12)
    ...
    while (true) {
        ...
        if (bossBeaming && beamTimer > 0) {
            ...
            if (boss.y >= 70 && !enemies[beamEnemy].hasCaptured) {
                ...
                // No blocking delay — explosion plays out via updateExplosions()
            }
            if (beamTimer <= 0) {
                bossBeaming = false;
                enemies[beamEnemy].state = ES_FORMATION;
